...
def setup_db():
    ...
    except Exception as e: print("DB Setup Error:", e)
...
def register(message):
    ...
    except: bot.send_message(message.chat.id, "❌ রেজিস্ট্রেশনে সমস্যা হয়েছে।")
...
def get_action_buttons(uid, show_work=True):
    ...
    kb.add(types.InlineKeyboardButton("✅ Approve", callback_data=f"act_app_{uid}"), types.InlineKeyboardButton("❌ Reject", callback_data=f"act_rej_{uid}"))
    if show_work: kb.add(types.InlineKeyboardButton("⚙️ Work", callback_data=f"act_wrk_{uid}"))
    return kb

@bot.callback_query_handler(func=lambda c: c.data.startswith('act_'))
def handle_action_buttons(call):
    if not is_admin(call.from_user): return bot.answer_callback_query(call.id, "⛔ অনুমতি নেই!")
    parts = call.data.split('_'); action, uid = parts[1], int(parts[2]); admin_name = call.from_user.first_name; msg = call.message
    try:
        if action == 'wrk':
            new_kb = get_action_buttons(uid, show_work=False); status_text = "\n\n⚙️ <b>Status:</b> <i>Working on it...</i>"
            if msg.content_type == 'photo': bot.edit_message_caption(caption=(msg.caption or "") + status_text, chat_id=msg.chat.id, message_id=msg.message_id, reply_markup=new_kb, parse_mode="HTML")
            else: bot.edit_message_text(text=(msg.text or "") + status_text, chat_id=msg.chat.id, message_id=msg.message_id, reply_markup=new_kb, parse_mode="HTML")
        elif action == 'app':
            bot.send_message(uid, "✅ <b>আপনার রিকোয়েস্টটি অ্যাডমিন দ্বারা Approve করা হয়েছে!</b>")
            final_text = "\n\n✅ <b>Status:</b> Approved by " + admin_name
            if msg.content_type == 'photo': bot.edit_message_caption(caption=(msg.caption or "").replace("Working on it...", "") + final_text, chat_id=msg.chat.id, message_id=msg.message_id, reply_markup=None, parse_mode="HTML")
            else: bot.edit_message_text(text=(msg.text or "").replace("Working on it...", "") + final_text, chat_id=msg.chat.id, message_id=msg.message_id, reply_markup=None, parse_mode="HTML")
        elif action == 'rej':
            bot.send_message(uid, "❌ <b>আপনার রিকোয়েস্টটি অ্যাডমিন দ্বারা Reject করা হয়েছে।</b>")
            final_text = "\n\n❌ <b>Status:</b> Rejected by " + admin_name
            if msg.content_type == 'photo': bot.edit_message_caption(caption=(msg.caption or "").replace("Working on it...", "") + final_text, chat_id=msg.chat.id, message_id=msg.message_id, reply_markup=None, parse_mode="HTML")
            else: bot.edit_message_text(text=(msg.text or "").replace("Working on it...", "") + final_text, chat_id=msg.chat.id, message_id=msg.message_id, reply_markup=None, parse_mode="HTML")
    except: bot.answer_callback_query(call.id, "অ্যাকশন এরর!")
...
def save_hourly(message):
    if is_cmd(message): return
    if not message.photo: return bot.send_message(message.chat.id, "❌ স্ক্রিনশট বাধ্যতামূলক।")
    name, cap = get_user_name(message.chat.id), clean_text(message.caption)
    calls_h, nsu_h = 0, 0
    try:
    ...
    except: bot.send_message(message.chat.id, "❌ এরর হয়েছে।")
...
def handle_dep_with(call):
    ...
    is_dep = (call.data == "req_dep"); fmt = "ইউজার নাম: \nমোবাইল নাম্বার: \nট্রানজেকশন আইডি: \nনোট: " if is_dep else "ইউজার নাম: \nমোবাইল নাম্বার: \nলেনদেন আইডি: \nঅ্যামাউন্ট: \nনোট: "
...
def save_transaction(message, req_type):
    if is_cmd(message): return
    if req_type == "DEPOSIT" and not message.photo: return bot.send_message(message.chat.id, "❌ স্ক্রিনশট লাগবে।")
    name, cap = get_user_name(message.chat.id), clean_text(message.caption if message.photo else message.text)
    report = f"💰 <b>{req_type} REQUEST</b>\n👤 User: {name}\n📢 <b>Admin:</b> {ADMIN_MENTION}\n📝 Details:\n{cap}"
    act_kb = get_action_buttons(message.chat.id)
    try:
        if message.photo: bot.send_photo(ADMIN_GROUP_ID, message.photo[-1].file_id, caption=report, reply_markup=act_kb, message_thread_id=TOPIC_PAYMENT)
        else: bot.send_message(ADMIN_GROUP_ID, report, reply_markup=act_kb, message_thread_id=TOPIC_PAYMENT)
...
def attend_menu(message):
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(types.InlineKeyboardButton("🟢 ডিউটি শুরু", callback_data="sw"), types.InlineKeyboardButton("⏸️ বিরতি", callback_data="bw"))
...
def attend_call(call):
    ...
    uid, name, now = call.message.chat.id, get_user_name(call.message.chat.id), bd_time()
    t_str, d_str = now.strftime("%Y-%m-%d %H:%M:%S"), now.strftime("%Y-%m-%d")
    conn = get_conn(); cursor = conn.cursor()
    ...
    row = cursor.fetchone(); c_status, s_str = row[0], row[1]
    if call.data == 'sw':
        ...
        if c_status == 'working':
            try:
                st = datetime.strptime(s_str, "%Y-%m-%d %H:%M:%S"); sec = max(int((now - st).total_seconds()), 0)
...
def recharge_app(message):
    fmt = "তারিখ: \nপেমেন্ট সিস্টেম: \nএমাউন্টও: "; msg = bot.send_message(message.chat.id, f"📱 <b>Recharge</b>\n\n<code>{fmt}</code>\n\nস্ক্রিনশটসহ দিন।")
    bot.register_next_step_handler(msg, save_recharge)

def save_recharge(message):
    if is_cmd(message) or not message.photo: return bot.send_message(message.chat.id, "❌ স্ক্রিনশট বাধ্যতামূলক।")
    name, cap = get_user_name(message.chat.id), clean_text(message.caption); act_kb = get_action_buttons(message.chat.id)
...
def leave_menu(message):
    ...
    kb.add(types.InlineKeyboardButton("🤒 অসুস্থ ছুটি", callback_data="lv_sick"), types.InlineKeyboardButton("⏳ অতিরিক্ত বিরতি সময়", callback_data="lv_extra"), types.InlineKeyboardButton("🆘 ইমারজেন্সি কাজ", callback_data="lv_emg"))
...
def handle_leave(call):
    if call.data == "lv_sick": f, m = "তারিখ: \nবিস্তারিত: \nদিন: ", "SICK LEAVE"
    elif call.data == "lv_extra": f, m = "বিরতি শুরু: \nবিরতি শেষ: \nমোট সময়: ", "EXTRA BREAK"
    else: f, m = "তারিখ: \nকারণ: \nডকুমেন্টস: ", "EMERGENCY WORK"
...
def save_leave(message, mode):
    if is_cmd(message): return
    if mode == "EMERGENCY WORK" and not message.photo: return bot.send_message(message.chat.id, "❌ স্ক্রিনশট লাগবে।")
    name, cap = get_user_name(message.chat.id), clean_text(message.caption if message.photo else message.text); act_kb = get_action_buttons(message.chat.id); report = f"🩺 <b>{mode}</b>\n👤 {name}\n📢 {ADMIN_MENTION}\n📝 {cap}"
    if message.photo: bot.send_photo(ADMIN_GROUP_ID, message.photo[-1].file_id, caption=report, reply_markup=act_kb, message_thread_id=TOPIC_LEAVE)
    else: bot.send_message(ADMIN_GROUP_ID, report, reply_markup=act_kb, message_thread_id=TOPIC_LEAVE)
    bot.send_message(message.chat.id, "✅ পাঠানো হয়েছে।")
...
def admin_panel_menu(message):
    ...
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(types.InlineKeyboardButton("📋 রিপোর্ট চেক", callback_data="adm_check"), types.InlineKeyboardButton("🏆 Best Performer ঘোষণা", callback_data="adm_best"), types.InlineKeyboardButton("👤 ইউজার ম্যানেজমেন্ট", callback_data="adm_manage"), types.InlineKeyboardButton("📢 প্রমোশন মেসেজ", callback_data="adm_promo"), types.InlineKeyboardButton("💬 মেনশন মেসেজ", callback_data="adm_mention"), types.InlineKeyboardButton("📢 আপডেট নোটিশ পাঠান", callback_data="adm_upd_not"))
...
def handle_adm_callback(call):
    ...
    if call.data == "adm_upd_not":
        msg = bot.send_message(call.message.chat.id, "📢 আপডেট নোটিশটি লিখুন:"); bot.register_next_step_handler(msg, broadcast_promo)
    elif call.data == "adm_mention":
        conn = get_conn(); cur = conn.cursor(); cur.execute("SELECT user_id, name FROM users"); u = cur.fetchall(); conn.close()
        kb = types.InlineKeyboardMarkup(); [kb.add(types.InlineKeyboardButton(x[1], callback_data=f"mnt_{x[0]}")) for x in u]
        ...
        msg = bot.send_message(call.message.chat.id, "🏆 <b>গতকালকের</b> সেরা পারফর্মার লিখুন (ছবিসহ):"); bot.register_next_step_handler(msg, broadcast_best)
    elif call.data == "adm_promo":
        msg = bot.send_message(call.message.chat.id, "📢 প্রমোশন মেসেজটি লিখুন:"); bot.register_next_step_handler(msg, broadcast_promo)
    elif call.data == "adm_manage":
        conn = get_conn(); cur = conn.cursor(); cur.execute("SELECT user_id, name FROM users"); u = cur.fetchall(); conn.close()
        kb = types.InlineKeyboardMarkup(); [kb.add(types.InlineKeyboardButton(f"❌ Remove: {x[1]}", callback_data=f"del_{x[0]}")) for x in u]
        bot.edit_message_text("👤 ইউজার রিমুভ:", call.message.chat.id, call.message.message_id, reply_markup=kb)
    elif call.data == "adm_check":
        conn = get_conn(); cur = conn.cursor(); cur.execute("SELECT user_id, name FROM users"); u = cur.fetchall(); conn.close()
        kb = types.InlineKeyboardMarkup(); [kb.add(types.InlineKeyboardButton(x[1], callback_data=f"rpt_{x[0]}")) for x in u]
...
def mnt_step_2(call):
    uid = call.data.split("_")[1]; msg = bot.send_message(call.message.chat.id, "💬 মেনশন মেসেজ লিখুন:"); bot.register_next_step_handler(msg, lambda m: send_mnt(m, uid))
...
def send_mnt(message, uid):
    ...
    try:
        if message.photo: bot.send_photo(uid, message.photo[-1].file_id, caption=txt)
        else: bot.send_message(uid, txt); bot.send_message(message.chat.id, "✅ মেনশন পাঠানো হয়েছে।")
    except: pass

def broadcast_best(message):
    if is_cmd(message): return
    txt = f"🌟 <b>সেরা পারফর্মার!</b> 🌟\n━━━━━━━━━━━━━━━━━━\n{clean_text(message.caption if message.photo else message.text)}"; send_to_all(txt, message.photo)
...
def send_to_all(txt, photo=None):
    conn = get_conn(); cur = conn.cursor(); cur.execute("SELECT user_id FROM users"); u = cur.fetchall(); conn.close()
    for x in u:
        try:
            if photo: bot.send_photo(x[0], photo[-1].file_id, caption=txt)
            else: bot.send_message(x[0], txt)
        except: pass

@bot.callback_query_handler(func=lambda c: c.data.startswith("del_"))
def del_u(call):
    uid = call.data.split("_")[1]; conn = get_conn(); cur = conn.cursor(); cur.execute("DELETE FROM users WHERE user_id=%s", (uid,)); conn.commit(); conn.close(); bot.edit_message_text("✅ ইউজার রিমুভড।", call.message.chat.id, call.message.message_id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("rpt_"))
def rpt_range(call):
    uid = call.data.split("_")[1]; kb = types.InlineKeyboardMarkup(row_width=2); kb.add(types.InlineKeyboardButton("২৪ ঘণ্টা", callback_data=f"dr_{uid}_1"), types.InlineKeyboardButton("৭ দিন", callback_data=f"dr_{uid}_7"), types.InlineKeyboardButton("১৫ দিন", callback_data=f"dr_{uid}_15"), types.InlineKeyboardButton("৩০ দিন", callback_data=f"dr_{uid}_30"))
...
def rpt_final(call):
    p = call.data.split("_"); uid, days = int(p[1]), int(p[2]); target = (bd_time() - timedelta(days=days-1)).strftime("%Y-%m-%d")
    conn = get_conn(); cur = conn.cursor(); cur.execute("SELECT name FROM users WHERE user_id=%s", (uid,)); n = cur.fetchone()[0]
    cur.execute("SELECT COALESCE(SUM(total_seconds),0) FROM work_hours WHERE user_id=%s AND date >= %s", (uid, target)); s = cur.fetchone()[0]
    cur.execute("SELECT COALESCE(SUM(calls_h),0), COALESCE(SUM(nsu_h),0), COUNT(*) FROM hourly_stats WHERE user_id=%s AND date >= %s", (uid, target)); st = cur.fetchone(); conn.close()
    h, m = s // 3600, (s % 3600) // 60; bot.edit_message_text(f"📊 <b>Report: {n}</b>\n⏳ মোট কাজ: {h} ঘণ্টা {m} মিনিট\n📑 Hourly: {st[2]} বার\n📞 Calls: {st[0]} | 📉 NSU: {st[1]}", call.message.chat.id, call.message.message_id)

# =======================================================
# ⏰ অটোমেশন (Alerts & Warnings)
# =======================================================
def automation():
    best_alert = False; greet = {"m": False, "a": False, "e": False}
    while True:
        try:
            now = bd_time()
            if now.hour == 9 and now.minute == 0 and not best_alert: bot.send_message(ADMIN_GROUP_ID, f"🔔 {ADMIN_MENTION} <b>গতকালকের Best Performer</b> ঘোষণার সময় হয়েছে।"); best_alert = True
            if now.hour == 0: best_alert = False; greet = {"m": False, "a": False, "e": False}

            if now.hour == 8 and now.minute == 0 and not greet["m"]: send_to_all("☀️ <b>শুভ সকাল!</b>\nকাজে মন দিন এবং আজকের লক্ষ্য পূরণ করুন। 🚀"); greet["m"] = True
            if now.hour == 14 and now.minute == 0 and not greet["a"]: send_to_all("🌤️ <b>শুভ দুপুর!</b>\nসাফল্যের জন্য আপনার চেষ্টাই যথেষ্ট। এগিয়ে যান! 💪"); greet["a"] = True
            if now.hour == 18 and now.minute == 0 and not greet["e"]: send_to_all("🌆 <b>শুভ সন্ধ্যা!</b>\nপেশাদারিত্বের সাথে পরিশ্রম চালিয়ে যান। ✨"); greet["e"] = True

            conn = get_conn(); cur = conn.cursor(); cur.execute("SELECT a.user_id, a.status, a.last_break_time, a.start_time, a.last_report_time, u.name, u.username FROM attendance a JOIN users u ON a.user_id = u.user_id"); rows = cur.fetchall()
            for u, s, lb, st, lr, name, username in rows:
                ...
                if not is_u_admin:
                    if s == 'break' and lb:
                        ...
                        if 3000 <= diff <= 3060: bot.send_message(u, "🔔 বিরতির ৫০ মিনিট পূর্ণ। কাজে ফেরার সময় হয়েছে। 😊")
                        if 3600 <= diff <= 3660: bot.send_message(ADMIN_GROUP_ID, f"⚠️ <b>Warning:</b> {name} ১ ঘণ্টার বেশি সময় ধরে বিরতিতে!")
                    if s == 'working' and lr:
                        diff_report = (now - datetime.strptime(lr, "%Y-%m-%d %H:%M:%S")).total_seconds()
                        if 5400 <= diff_report <= 5460: # ১ ঘণ্টা ৩০ মিনিট (৯০ মিনিট)
                    ...
                    if s == 'working' and st and now.hour == 22 and now.minute == 0:
                        if datetime.strptime(st, "%Y-%m-%d %H:%M:%S").hour == 10: bot.send_message(u, "📢 সময় শেষ! শেষ রিপোর্ট দিয়ে ডিউটি শেষ করুন।")
...
app = Flask(__name__)
@app.route('/')
def home(): return "🤖 Bot is Online with Admin Protection & Updated Warning!"

def run_bot():
    bot.remove_webhook()
    while True:
        try: bot.infinity_polling(timeout=20, skip_pending=True)
        except: time.sleep(5)
