...
def save_hourly(message):
    ...
    if not message.photo:
        return bot.send_message(message.chat.id, "❌ <b>ভুল হয়েছে!</b> ছবি/স্ক্রিনশট ছাড়া রিপোর্ট সাবমিট হবে না।", reply_markup=main_menu(message.from_user))
        ...
        with db_lock:
            ...
            # ⏸️ বিরতি
                ...
                if current_status == 'break':
                    ...
                    try: bot.send_message(ADMIN_GROUP_ID, f"⏸️ <b>Break Started (বিরতি)</b>\n👤 {name}\n⏰ {disp_time}", message_thread_id=TOPIC_ATTENDANCE)
                    except: pass
...
def admin_panel_menu(message):
    ...
    kb.add(
        ...
        types.InlineKeyboardButton("🏆 Daily Best Performer", callback_data="admin_best"),
...
def handle_admin_actions(call):
    ...
    if call.data == "admin_check":
        ...
        if best and best[0]:
            txt = f"🏆 <b>Daily Best Performer</b> 🏆\n\n📅 Date: [{display_date}]\n👤 Name: {best[0]}\n📈 Result: NSU – {best[1]} | FTD/Call – {best[2]}\n\nKeep pushing and congratulations to today’s top performer! 🚀"
            try: bot.send_message(ADMIN_GROUP_ID, txt)
            except: pass
            for u in all_users:
                try: bot.send_message(u[0], txt)
                except: pass
            bot.send_message(call.message.chat.id, "✅ বেস্ট পারফর্মার মেসেজ সবার কাছে পাঠানো হয়েছে।")
        else:
            bot.send_message(call.message.chat.id, "আজকের কোনো ডাটা পাওয়া যায়নি।")
            
    elif call.data == "admin_promo":
        msg = bot.send_message(call.message.chat.id, "📢 <b>প্রমোশন মেসেজ:</b>\nযে নোটিশটি সবার কাছে পাঠাতে চান, তা লিখে সেন্ড করুন:")
...
def send_promotion(message):
    ...
    bot.send_message(message.chat.id, f"✅ প্রমোশন মেসেজটি সফলভাবে {count} জনের কাছে পাঠানো হয়েছে।")
                ...
                with db_lock:
                    ...
                    for u, lr in cursor.fetchall():
                        if lr and (now - datetime.strptime(lr, "%Y-%m-%d %H:%M:%S")).total_seconds() >= 3600:
                            ...
                            # রিমাইন্ডার পাঠানোর পর টাইম আপডেট
...
try:
    bot.remove_webhook()
    time.sleep(1)
    # Long polling with error handling so it never crashes
